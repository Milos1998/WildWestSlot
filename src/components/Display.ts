...
import { DISPLAYS_COLOR, DISPLAYS_FRAME_COLOR, DISPLAYS_FRAME_THICKNESS, DISPLAYS_FONT_FAMILY, DISPLAYS_FONT_FILL, DISPLAYS_DISABLED_BUTTON_COLOR } from "../constants/constants";
...
let displayValueStyle= new TextStyle({
    fill: DISPLAYS_FONT_FILL,
    fontFamily: DISPLAYS_FONT_FAMILY,
    fontSize: 25,
})
...
export class Display extends Graphics{
    ...
    constructor(x: number, y: number, width: number, height: number, _displayValue: string, _description: string, isSelector: boolean= false){
        super()
    ...
    public enableSelector(){
        if(this.wasLessActive)
            this.enableLessButton()
        if(this.wasMoreActive)
